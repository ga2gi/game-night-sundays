<script>
  ...
  const games = [
...

    {
      ...
      description: 'Socialite Naomi Mwangi was found dead inside her apartment after a gathering with close friends. Every suspect has a motive. Every timeline has contradictions.',
  ...
  /* 
    THESE ARE THE COMMUNITY ACCESS CODES
  ...
  */
  ...
  function verifyAccess(gameId) {
    if (inputCode.toUpperCase() === masterCodes[gameId]) {
      ...
      alert('ACCESS DENIED // INVALID COMMUNITY CODE');
      inputCode = '';
    }
  }
  ...
  <header class="archive-header">
    ...
    <div class="top-meta">
      <span class="system-status">
        SYSTEM_ONLINE // ENCRYPTION_ACTIVE
      </span>
    ...
    <div class="title-block">
      <h1>
        CENTRAL <span class="accent">ARCHIVE</span>
      </h1>

      <div class="stats-ribbon">
        <div class="stat">
          <span>TOTAL_FILES:</span> {stats.total}
        </div>

        <div class="stat">
          <span>PREMIUM_CLEARANCE:</span> {stats.premium}
        </div>

        <div class="stat">
          <span>COMMUNITY_FILES:</span> {stats.free}
        </div>
      </div>
    </div>
  </header>
  ...
  <section class="console-panel">
    ...
    <div class="input-group">
      <div class="search-bar">
        ...
        <input
          type="text"
          placeholder="ENTER TITLE OR REGION..."
    ...
    {#each filteredGames as game, i (game.id)}
      ...
      >
        ...
        <div class="card-visual">
          ...
          <div
            class="price-tag"
            class:is-free={game.category === 'free'}
          >
            {game.price}
          </div>
        ...
        <div class="card-body">
          ...
          <div class="footer-actions">
            ...
            <!-- PREMIUM GAMES -->
            {#if game.category === 'premium'}

              <a
                href={`/games/${game.id}`}
                class="investigate-btn premium-btn"
              >
                PURCHASE DOSSIER
              </a>

            <!-- FREE COMMUNITY GAMES -->
            {:else if unlockedCases.includes(game.id)}

              <a
                href={`/games/${game.id}`}
                class="investigate-btn granted"
                in:fade
              >
            ...
            {:else if activeVerificationId === game.id}
              ...
              <div class="access-gate" in:slide>
                ...
                <input
                  ...
                  onkeydown={(e) =>
                    e.key === 'Enter' &&
                    verifyAccess(game.id)}
                  autofocus
                />

                <button onclick={() => verifyAccess(game.id)}>
                  VERIFY
                </button>

                <button
                  class="cancel"
                  onclick={() => (activeVerificationId = null)}
                >
                  ×
                </button>
            ...
            {:else}
              ...
              <button
                ...
                onclick={() =>
                  (activeVerificationId = game.id)}
              >
                UNLOCK COMMUNITY FILE
              </button>
